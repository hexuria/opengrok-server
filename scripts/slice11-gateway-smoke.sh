...
frames=$(curl -sN --max-time 2 "$BASE/events?channels=agents,transcript" "${IDENT[@]}" -H 'accept: text/event-stream' || true)
case "$frames" in
  "retry: 1000"*) ok "retry: 1000 is the first line" ;;
  *) fail "the stream did not open with retry: 1000: $(echo "$frames" | head -2)" ;;
esac
ctype=$(curl -sN -o /dev/null --max-time 2 -w '%{content_type}' "$BASE/events" "${IDENT[@]}" || true)
case "$ctype" in
...
pinged=$(curl -sN --max-time 12 "$BASE/events" "${IDENT[@]}" || true)
...
resp_headers=$(curl -fsS -D - -o /dev/null -X POST "$BASE/api/getTrays" "${IDENT[@]}" -H 'content-type: application/json')
...
code=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/api/countAgents" "${IDENT[@]}")
...
echoed=$(curl -fsS -X POST "$BASE/api/setHostSettings" "${IDENT[@]}" -H 'content-type: application/json' \
...
after=$(curl -fsS -X POST "$BASE/api/getHostSettings" "${IDENT[@]}")
...
missing=$(curl -s -w '\n%{http_code}' -X POST "$BASE/api/noSuchThing" "${IDENT[@]}")
...
curl -fsS -X POST "$BASE/coworkers" -H "authorization: Bearer $ACCOUNT_TOKEN" \
  -H 'content-type: application/json' -d '{"name":"Gateway Smoke","model":"oag/cheap"}' >/dev/null
rows=$(curl -fsS -X POST "$BASE/api/listAgents" "${IDENT[@]}")
