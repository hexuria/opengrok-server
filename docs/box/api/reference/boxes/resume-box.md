...
````yaml openapi/box-v1.yaml POST /boxes/{boxId}/resume
openapi: 3.1.0
info:
...
components:
  ...
  schemas:
    ResumeRequest:
      type: object
      description: Options for resuming a stopped Box.
      properties:
        type:
          type: string
          enum:
            ...
            - large
          description: >-
            Resume onto a different machine size. Omit to keep the box's current
            type. A resume already restores onto a fresh machine, so changing
            size costs nothing extra. Shrinking is rejected with
            `type_too_small` when the box's data would not fit the smaller disk.
        env:
          type: object
          additionalProperties:
            type: string
          description: >-
            Replaces the box's per-box environment variables. Omit to keep the
            box's current env. Same validation rules as `CreateBoxRequest.env`.
        environment:
          type: string
          description: >-
            Optionally re-pin the box to a different named Box environment on
            resume. Omit to keep the box's current environment. Unknown names
            are rejected with `unknown_environment`.
          examples:
            - base
            - customer-demos
        noEnv:
          type: boolean
          description: >-
            Resume into a no-env box: withhold account secrets and scrub
            inherited owner secrets from the restored snapshot before the box is
            exposed. One-way; once a box is resumed as no-env it stays no-env on
            later resumes.
        ttlSeconds:
          oneOf:
            - type: integer
            ...
            - type: 'null'
          description: >-
            Auto-stop for the resumed Box, in seconds, stored on the Box. Omit
            to keep the TTL the Box already had. `null` disables auto-stop,
            which means nothing will ever stop this Box for you.
      examples:
        - ttlSeconds: 28800
        - type: large
          ttlSeconds: 3600
