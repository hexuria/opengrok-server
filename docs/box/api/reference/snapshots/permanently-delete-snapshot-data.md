...
> Confirm and accept irreversible background deletion of a snapshot.



## OpenAPI

````yaml openapi/box-v1.yaml DELETE /snapshots/{snapshotId}
openapi: 3.1.0
info:
...
paths:
  /snapshots/{snapshotId}:
    delete:
      ...
      description: >-
        Accept irreversible background deletion of an unpinned snapshot owned by
        the authenticated account. Snapshots required by another incremental
        snapshot or active restore return `409`. Poll the returned operation
        until `completed`.
