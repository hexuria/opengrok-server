...
| `OG_GATEWAY_TOKEN` | — | an `oag_live_…` key. **Never a provider key** — a pin is a route, not a credential (CLAUDE.md #4). Probed at boot with `GET /v1/models`, which bills nothing: a key the gateway refuses (401/403) stops the boot with that sentence; a gateway that does not answer is only a warning, since it may be starting after us. `/health` still answers for the event store only |
