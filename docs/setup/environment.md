...
| `OG_CONTEXT_TOKENS` | `128000` | the context a turn is held to when the gateway's catalogue does not say (`oag.context_window` on `/v1/models`, read by pin, then by `alias_of`, then by the pin without `@sub`/`@api`). Before each model call the harness estimates the prompt (UTF-8 bytes / 3, a fixed cost per picture), keeps room for the answer, and leaves out the oldest whole turns of the conversation until it fits — down to 80% of the room, so the prompt's prefix does not change every turn — telling the model in its system message how many it left out. This turn, and on a resumed run everything the run did, is never left out; a request that still does not fit is not sent, and the run ends with a sentence naming the model instead of the provider's 400. `0` turns the guard off. The default is set because the shipping pin `xai/grok-4.6` is served but never advertised (#90) |
