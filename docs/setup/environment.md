...
| `OG_PUBLIC_GATEWAY_URL` | unset | the **non-loopback** address `EnsureSandBox` mints to clients (e.g. `http://192.168.100.24:1447`). The client refuses a loopback gateway host, so the mint is refused outright when this is loopback or unset |
