...
| `OG_MODEL` | `xai/grok-4.6` | the route a NEW coworker is hired on when none is named. **The default is chosen for making real tool calls**: `xai/grok-4.6` is the route on the record doing so ([policy-card capture](../verification/policy-card/README.md#model-note)); the old default `gpt-5.6-luna` answers shell requests with invented text and zero tool calls, so a coworker on it cannot use its computer. The console's **Test** (`POST /models/probe`) offers one tool and answers `toolCalls: true\|false`; `false` with `ok: true` means the route talks but cannot act. Each coworker then keeps its own pin (changeable in the console at `/console/coworkers`), so changing this retargets nothing existing — a coworker hired on the old luna default stays on it until repinned there. Dialect: `provider/model` (`openai/gpt-5.5`), `@api`/`@sub`, or a ladder id (`oag/auto`); a bare name works on a passthrough route. **Servable and advertised are independent, in both directions.** An advertised id is not necessarily servable — `oag/auto` is refused on a route with no credential for the rung it picks; `POST /models/probe` proves a pin before it is saved. And the reverse: a **servable id need not be advertised** — `/v1/models` is built from each provider's own model listing, and xAI's returns quota with no model list, so `xai/grok-4.6` serves perfectly while never appearing in the picker. Do not "fix" a working default because the picker does not list it |
| `OG_AUTO_REVIEW_MODEL` | `OG_MODEL` | the auto-review judge's route — deliberately not the coworker's own route (the reviewer must not be the reviewed). **Blank counts as unset** (`.env.example` ships it blank), and the fallback puts a default-hired coworker under its own review, so name a different, cheap route here; the judge offers no tools, so a route that only talks is fine. Billed to the coworker's key, so a capped coworker's judge is refused: its cards say so, the log line names coworker, model and call, and after 3 failures in a row the run refuses reviewed calls instead of carding each one |
| `OG_CONTEXT_TOKENS` | `128000` | the context a turn is held to when the gateway's catalogue does not say (`oag.context_window` on `/v1/models`, read by pin, then by `alias_of`, then by the same model on another channel, the smallest window when several match). Before each model call the harness estimates the prompt (UTF-8 bytes / 3, a fixed cost per picture), keeps room for the answer, and leaves out the oldest whole turns of the conversation until it fits — down to 80% of the room, so the later rounds of that run keep one prefix for the provider's prompt cache — telling the model in its system message how many it left out. This turn, a client's own leading `system` message, and on a resumed run everything the run did, are never left out; a request that still does not fit is not sent, and the run ends with a sentence naming the model instead of the provider's 400. `0` turns the guard off. The default is set because the shipping pin `xai/grok-4.6` is served but never advertised (#90) |
...
| `OG_TURN_TIMING` | unset (compact always) | Compact AG-UI CUSTOM `run-timing` is **always** emitted on `RUN_FINISHED` / `RUN_ERROR` (model_ms, tools, tool_wait_ms, auto_review_ms, total_ms, tool_rounds; `budget`, `wrapped_up` when set; `context`: `{limit, estimated: [one per model call], left_out}`) plus one INFO line. `1` / `true` also logs the JSON body. NativeChat can hang the compact frame in a debug drawer |
