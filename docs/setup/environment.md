...
| `OG_DOCKER_IMAGE` | `debian:stable-slim` | the image a Docker computer is built from; any image with a shell. A desktop image (`grok-box`) publishes noVNC on `127.0.0.1` only, with its own random 8-character `BOX_VNC_PASSWORD` (read back from the container's env after a restart, like `BOX_TOKEN`). The app never gets that loopback URL: `vncUrl` is `<origin>/coworkers/{id}/computer/vnc/{ticket}/vnc.html?…`, the page and its websocket proxied by OpenGrok, where `<origin>` is `OG_PUBLIC_GATEWAY_URL` or, when that is a `0.0.0.0` listen address, the `Host` the app used. The ticket names one account, coworker and box, lives 6–12 h, and the account's right to the coworker is checked again on every request. The URL carries the ticket and the password — do not log it |
