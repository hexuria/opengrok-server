...
**What was removed, 18 Sep 2026.** The two doors that existed only to boot the reconstructed
...
went with them. Whether the desktop client is dead is a product decision the operator has not
made, so this removal is a deletion git can revert, not a verdict on the client.
...
1. **No vendored generated protobuf stubs.** `source/packages/proto/generated/**` is never copied
   ...
   `.proto` text or prost output; `crates/opengrok-proto` was deleted with seam B on 18 Sep 2026.
