...
| Model door | Every model call exits through open-ai-gateway (`oag_live_` key, OpenAI-compatible routes). **One real door behind `ModelDoor`:** `GatewayDoor`, plus the mocks. A `rig-core` door existed and was retired on 17 Sep 2026 — it did not put the model on the wire, so every coworker's pin was silently ignored, which is what cost a night on 8 Sep 2026 (the two dates are the bug and its removal, not one event). The trait stays; a second door is a file, not a rewrite, if one is ever earned. MCP comes from `rmcp` directly, never from rig. |
