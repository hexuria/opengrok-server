...
| `/ag-ui/threads` | GET | `agui/routes.rs:911` | JSON array of threads, newest first (`?coworkerId=&limit=&before=&beforeThreadId=`); 401 `{error}` (checked first); 400 `{error}` for a non-numeric `limit`/`before` or `beforeThreadId` without `before`; 503 `{error}`. The order follows latest activity, so a client paging while turns land de-duplicates by `threadId` | not yet — NativeChat issue filed after merge |
...
| `/local-exec/policy` | GET PUT | `local_exec.rs:234` | GET: `{machineId, mode, allow: [string], deny: [string], inert: [{pattern, reason}]}` — `inert` names the allow rows the gate can never match, which stay listed in `allow` (#223); PUT: 204 (422 "unknown mode") | `ensure_daemon` writes `ask` on first enrol; the card's Always/Never write `bypass`/`never` — `src/opengrok/local_exec.rs`, `src/state.rs:6001–6004` (#147, 19 Sep 2026; not re-read) |
