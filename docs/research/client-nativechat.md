...
| `/ag-ui/threads/{thread_id}` | GET | `agui/routes.rs:893` | the thread replayed from the log (`?limit=&events=`); 404 "no such thread" | `?events=true` on a cold load — `docs/review/findings-og139.md`. NativeChat file: *unverified* |
