...
2. Require `hello.app` to be `bir-desktop` (or the BIR app id). If `nativechat` or another app owns the port, pick a free port (often `17423`) and export matching `GPUI_AGENT_ADDR` for the rest of this turn.
...
Prefer host invokes from AGENT.md: `profile.search` / `profile.list` / `profile.set`, `profile.forms_set.get`, `dues.list`, `nav.go`, `form.*`, `filing.validate`. Never `profile.ensure`. `profile.save` and queue only after Uriah confirms. Never skip lock screen / PIN / TOTP.
