...
- [x] **A person's conversations are listed by the server.** `GET /ag-ui/threads?coworkerId=&limit=&before=&beforeThreadId=` answers the bearer's own threads as a JSON array, newest first: `{threadId, coworkerId, origin, title, lastRunId, lastStatus, updatedAtMs}`, hidden turns left out, the MCP door's audit thread left out exactly, keyset-paged on `(updatedAtMs, threadId)`. Every part is owner-filtered, so two members on one org-shared coworker — the same thread id — each see their own last turn and their own first words as the title. One read-only query (`PgStore::threads_owned_by`), no schema change. Still owed: NativeChat building its sidebar from it on a fresh device. `tests/against_the_thread_list.rs`. #230.
- [x] **A conversation fits the model, or is not sent.** Before every model call — each round's and the wrap-up's — the harness estimates the prompt and, when it is over the model's context less room for the answer, leaves out the oldest whole turns (never this turn, never a resumed run's own work) and says so in the system message; what still does not fit ends the run with a sentence and no call, rather than the provider's 400 after a round trip. The limit is the gateway catalogue's `oag.context_window` for the pin, else `OG_CONTEXT_TOKENS` (default 128000, `0` off). Each call's estimate is on `run-timing` as `context`. Modelled as `TooLong` in `HarnessLoop`. Still owed: a model-written summary of what was left out, and clipping a single oversized tool result, which today can only end the run. `crates/opengrok-harness/src/context.rs`, `tests/against_a_long_thread.rs`. #90.
