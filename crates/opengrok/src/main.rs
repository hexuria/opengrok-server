...
async fn main() -> anyhow::Result<()> {
    ...
    let door: Arc<dyn ModelDoor> = match std::env::var("OG_MODEL_DOOR").as_deref() {
        ...
        // The fixture catalogue this door served left with seam A on 20 Sep 2026. Falling through
        // to the arm below would hand an operator who asked for a mock a real, billed door
        // (CLAUDE.md #8: fail closed and say why).
        Ok("mock-cards") => anyhow::bail!(
            "OG_MODEL_DOOR=mock-cards was removed with the desktop client's doors; choose \
             `mock`, `mock-tools`, or unset it to exit through the gateway."
        ),
        _ => {
