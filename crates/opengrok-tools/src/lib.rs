...
mod tests {
    ...
    /// Headless BIR runs on the sandbox when that image has it. A missing binary
    /// is the shell's own answer. The person's machine is a different tool.
    #[tokio::test]
    async fn a_box_shell_that_names_gpui_agent_reaches_the_sandbox() {
        ...
        assert_eq!(awake.ran(), 3, "the sandbox runs the command");

        let on_the_user_machine = executor
            .execute(
                &context,
                &call(
                    USER_MACHINE_SHELL,
                    json!({"command": "gpui-agent invoke profile.list"}),
                ),
            )
            .await;
        assert!(!on_the_user_machine.ok, "{on_the_user_machine:?}");
        assert!(
            !on_the_user_machine
                .content
