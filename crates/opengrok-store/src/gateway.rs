...
impl PgStore {
    ...
    /// A surgical flip of ONE field on the auto-review card's own path (`message.approval.status`).
    /// Touching one field keeps the summary/reason/command/proposedRule the user was shown intact
    /// — a whole-entry rewrite would need them back from a caller that never had them.
