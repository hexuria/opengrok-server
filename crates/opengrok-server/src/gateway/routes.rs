...
async fn command(
    ...
    match method.as_str() {
        ...
        "submitUserForm" => {
            let (code, body) =
                crate::user_form::submit_for_caller(&state.agui, &args, &caller).await;
            reply(
                StatusCode::from_u16(code).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
                body,
            )
        ...
        "dismissUserForm" => {
            let (code, body) =
                crate::user_form::dismiss_for_caller(&state.agui, &args, &caller).await;
            reply(
                StatusCode::from_u16(code).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
                body,
            )
        ...
        "resolveBoxHandoff" => {
            let (code, body) =
                crate::user_form::resolve_for_caller(&state.agui, &args, &caller).await;
            reply(
                StatusCode::from_u16(code).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
                body,
            )
        ...
        "submitCredentialResult" => {
            let (code, body) =
                crate::credential::result_for_caller(&state.agui, &args, &caller).await;
            reply(
                StatusCode::from_u16(code).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
                body,
            )
