//! HITL helpers the doors that stay need: UserForm / credential cards, parked-run
//! interrupt, resume after a card, the reply-quote line.
//!
//! These lived under `gateway/conversation.rs` because seam A minted the same cards.
//! NativeChat speaks AG-UI, so they belong next to that door, not inside a module that
...
async fn stop_parked_run(
    ...
    for _ in 0..5 {
        let Ok((mut run, seq)) = state.auth.store.load_run(run_id).await else {
            return false;
        };
...
/// Resume an approved AG-UI run: re-run the conversation with the approved tool call (which makes
...
///
/// `in_a_room` is accepted because the form/credential settlers still pass it (they shared a
/// resume with seam A's group path). NativeChat has no rooms; a member's turn is the coworker's
/// own transcript.
...
/// Resume an approved run: re-run the conversation with the approved tool call (which makes
/// `user_machine_shell` dispatch instead of re-asking), then land the model's summary in the
/// transcript as an ordinary bot message.
#[allow(clippy::too_many_arguments)]
async fn resume_agui_run(
    state: AgUiState,
    account_id: opengrok_core::id::AccountId,
    run_id: RunId,
    coworker_id: CoworkerId,
    ...
    if !text.is_empty() {
        ...
        if let Err(error) = state
            ...
            .await
        {
            tracing::error!(%error, "could not append the resumed turn's answer");
        }
    }
    ...
    let _ = emit_suspensions(&state, &coworker_id, &account_id, &agent_id, &events).await;
}
