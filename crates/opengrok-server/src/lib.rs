...
pub fn router(state: AgUiState) -> Router {
    ...
    // (the MCP door refuses those 403 before the token), and the LENGTH of the presented bearer
    ...
    // when it does not, and echoed on the response either way — so a client log line and a server
    // log line for the same call share one key. ORDER MATTERS: `.layer()` wraps what came before, so `Set` is added last
...
/// The longest client-supplied request id we keep. A UUID is 36. Past this the header is dropped before `SetRequestId` sees it, so a fresh id is minted instead —
