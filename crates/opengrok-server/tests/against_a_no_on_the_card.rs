...
//!
//! The desktop's door (seam A, deleted 20 Sep 2026) had resumed refusals from the start. Two doors
//! on to the same run disagreed; this is the one that was wrong, and the one that remains. So this
//! asserts what only a resumed refusal can be: the run REACHES AN ENDING, the tool never ran, and
//! the refusal the model was handed names the tool it may not use.
//!
...
impl Harness {
    ...
    /// One turn of `coworker` over `/ag-ui`, as its owner. The body is not read: the run is
    /// spawned server-side and journals itself, and the store is where this test looks.
    async fn turn(&self, access: &str, coworker: &str, prompt: &str) {
        let res = self
            .client
    ...
    /// Wait for a run of this person's to suspend on a card, and hand back the run and the call
    /// it is waiting on. Found the way a client finds it: off the queue of runs waiting on them.
    async fn wait_for_card(&self) -> (opengrok_core::id::RunId, String) {
        for _ in 0..100 {
            for id in self
                .store
        ...
        panic!("no run suspended on a card in 10s");
    }
}

#[tokio::test]
async fn a_no_on_the_card_is_told_to_the_model_and_ends_the_run() {
    let database_url = database_or_skip!();
    let email = format!("card-no-{}@og.local", uuid::Uuid::now_v7().simple());
    ...
    let (run_id, call_id) = h.wait_for_card().await;
    assert!(h.stub.ran().is_empty(), "nothing ran before the answer");

    // The answer route, and a no.
