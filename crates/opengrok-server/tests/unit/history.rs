...
fn an_earlier_turn_is_its_question_its_calls_and_its_answer() {
    ...
    let spoken: Vec<(&str, &str)> = messages
        .iter()
        .map(|message| (message.role.as_str(), message.content.as_str()))
        .collect();
    assert_eq!(spoken[0], ("user", "count the files"));
    assert!(
        spoken[1].1.contains("shell")
            && spoken[1].1.contains("ls")
            && spoken[1].1.contains("a b c"),
        "{spoken:?}"
    );
    assert_eq!(spoken[2], ("assistant", "three"));
    assert_eq!(
        spoken.len(),
        3,
        "a finished turn is not told to continue: {spoken:?}"
    );
}
